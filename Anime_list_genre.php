      ...
      <br><br>
       <div class="container" style="background-color: grey;margin-bottom: 30px;border-radius: 10px;padding: 10px;">
        <?php
          require_once("Dbconnect.php");
          $genre = $_GET['genre'];
          $query = "SELECT COUNT(genre) AS 'count' FROM Anime WHERE genre LIKE '%$genre%' ";
          $result = mysqli_query($con,$query);
          while ($row = mysqli_fetch_array($result)) {
          ?>
        <button class="btn btn-info" style="margin-left: 10px;padding: 10px;"><?php echo $_GET['genre']?> (<?=$row['count']?>)</button>
      <?php } ?>
        <br>
